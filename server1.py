# server1.py (with new Caching feature)
import socket
import os
import threading
...
def handle_client(conn, addr):
 ...
        elif s2_file_exists:
            # ### NEW FEATURE: CACHING LOGIC ###
            final_response = s2_content
            print("File only exists on SERVER 2. Sending to client.")
            try:
                print(f"[CACHE] Caching a local copy of '{filename}'...")
                with open(s1_filepath, 'w') as f:
                    f.write(s2_content)
                print(f"[CACHE] Successfully cached '{filename}' to {FILE_DIR}.")
            except Exception as e:
